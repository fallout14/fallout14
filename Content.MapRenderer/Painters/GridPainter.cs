...
namespace Content.MapRenderer.Painters
{
    public sealed class GridPainter
    {
        ...
        public void Run(Image gridCanvas, EntityUid gridUid, MapGridComponent grid, int offsetX = 0, int offsetY = 0)
        {
            ...
            if (_decals.TryGetValue(gridUid, out var decals))
                _decalPainter.Run(gridCanvas, CollectionsMarshal.AsSpan(decals), offsetX, offsetY);


            _entityPainter.Run(gridCanvas, entities, offsetX, offsetY);
